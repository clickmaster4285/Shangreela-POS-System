...
const LS_FLOORS = 'Shiraz Restaurant_floors';
const LS_TABLES = 'Shiraz Restaurant_tables';

function loadFloors(): FloorInfo[] {
  try {
    const raw = localStorage.getItem(LS_FLOORS);
    if (raw) {
      const p = JSON.parse(raw) as FloorInfo[];
      if (Array.isArray(p) && p.length > 0 && p.every(f => f.id && f.name)) return p;
    }
  } catch {
    /* ignore */
  }
  return defaultFloors.map(f => ({ ...f }));
}
...
function loadTables(): TableInfo[] {
  try {
    ...
    if (raw) {
      const p = JSON.parse(raw) as TableInfo[];
      if (Array.isArray(p) && p.length > 0) return p;
    }
  } catch {
    /* ignore */
  }
  return defaultTables.map(t => ({ ...t }));
}
...
export default function TableManagement() {
  ...
  const [floorsState, setFloorsState] = useState<FloorInfo[]>(loadFloors);
  const [tablesState, setTablesState] = useState<TableInfo[]>(loadTables);
  const [floorId, setFloorId] = useState<string>('all');

  useEffect(() => {
    localStorage.setItem(LS_FLOORS, JSON.stringify(floorsState));
  }, [floorsState]);
  useEffect(() => {
    localStorage.setItem(LS_TABLES, JSON.stringify(tablesState));
  }, [tablesState]);
  ...
  const saveFloor = () => {
    ...
    if (floorEditing) {
      setFloorsState(prev => prev.map(f => (f.id === floorEditing.id ? { ...f, name: n } : f)));
      toast.success('Floor updated');
    } else {
      const id = newFloorId(n, floorsState);
      setFloorsState(prev => [...prev, { id, name: n }]);
  ...
  const deleteFloor = (f: FloorInfo) => {
    ...
    setFloorsState(prev => prev.filter(x => x.id !== f.id));
  ...
  const saveTable = () => {
    ...
    if (tableEditing) {
      setTablesState(prev =>
        prev.map(t =>
          t.id === tableEditing.id
            ? { ...t, name, floorId: tableForm.floorId, seats, status: tableForm.status, currentOrder: tableForm.status === 'occupied' ? t.currentOrder : undefined }
            : t
        )
      );
      ...
      setTablesState(prev => [...prev, { id, name, floorId: tableForm.floorId, seats, status: tableForm.status }]);
  ...
  const deleteTable = (t: TableInfo) => {
    ...
    setTablesState(prev => prev.filter(x => x.id !== t.id));
